...
def output(potential, energies, wavefuncs, x_exp, sigma_x, xnew, first, last):#toomanyarguments
    """
    Save potential, eigenvalues, eigenfunctions, expactation value and uncertainty of x
    """
    np.savetxt('potential.dat', np.concatenate((np.reshape(xnew, (1999, 1)),
                                                np.reshape(potential(xnew), (1999, 1))), axis=1))
    np.savetxt('energies.dat', energies[first-1:last])
    np.savetxt('wavefuncs.dat', np.concatenate((np.reshape(xnew, (1999, 1)),
                                                np.transpose(wavefuncs[first-1:last])), axis=1))
    np.savetxt('expvalues.dat', np.concatenate((np.reshape(x_exp, (1999, 1)),
